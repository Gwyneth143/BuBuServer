...
const { touchBookUpdatedAtByOwner } = require("../services/db");
...
async function handleUpdateBook(req, res) {
  if (!(req.method === "POST" && getPathname(req.url) === "/books/update")) return false;
  ...
  try {
    const result = await touchBookUpdatedAtByOwner({ userId, bookId });
    if (result.notFound) {
      sendJson(res, 404, { error: "Book not found" });
      return true;
    }
    if (result.forbidden) {
      sendJson(res, 403, { error: "Forbidden: only the creator can update this book" });
      return true;
    }
